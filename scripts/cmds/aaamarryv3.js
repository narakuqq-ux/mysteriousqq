...
const BG_PATH    = path.join(CANVAS_DIR, "marriedv5.png");

const BG_URLS = [
  "https://i.ibb.co/mhxtgwm/49be174dafdc259030f70b1c57fa1c13.jpg",
  "https://i.postimg.cc/mhxtgwm/49be174dafdc259030f70b1c57fa1c13.jpg",
];
...
async function downloadBG() {
  ...
  for (const url of BG_URLS) {
    try {
      ...
      console.log("[marryv3] BG downloaded:", url);
      return true;
    } catch (e) {
      console.warn("[marryv3] BG fail:", url, e.message);
    }
  }
  return false;
}
...
async function makeImage({ one, two }) {
  await fs.ensureDir(CANVAS_DIR);
  const pathImg   = path.join(CANVAS_DIR, `marryv3_${one}_${two}.png`);
...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    description: { en: "Ikasal sa isang tao 💍 v3" },
    category: "img",
    guide: { en: "{pn} @mention" },
  },
  ...
  onStart: async function ({ api, event, usersData }) {
    ...
    try {
      ...
      console.error("[marryv3] Error:", err.message);
