...
const BG_PATH    = path.join(CANVAS_DIR, "hugv3.png");

const BG_URLS = [
  "https://i.imgur.com/7lPqHjw.jpg",
  "https://i.ibb.co/7lPqHjw/hugv3.jpg",
];
...
async function downloadBG() {
  ...
  for (const url of BG_URLS) {
    try {
      ...
      console.log("[hugv2] BG downloaded:", url);
      return true;
    } catch (e) {
      console.warn("[hugv2] BG fail:", url, e.message);
    }
  }
  return false;
}
...
async function makeImage({ one, two }) {
  await fs.ensureDir(CANVAS_DIR);
  const pathImg   = path.join(CANVAS_DIR, `hugv2_${one}_${two}.png`);
...
module.exports = {
  ...
  onStart: async function ({ api, event, usersData }) {
    ...
    if (!mentions[0])
      return api.sendMessage("❗ I-mention ang taong gusto mong i-hug!", threadID, messageID);
    ...
    try {
      ...
      await api.sendMessage(
        {
          body: `🥰 ${senderName} ay niyakap si @${targetName}!`,
      ...
      console.error("[hugv2] Error:", err.message);
