module.exports = {
  config: {
    ...
    countDown: 1,
    role: 0,
    description: { en: "No prefix auto-reply with sticker" },
    category: "events",
  },
  ...
  onChat: async function ({ api, event }) {
    ...
    if (
      lower.indexOf("bot") === 0 ||
      lower.indexOf("robot") === 0 ||
      lower.indexOf("hi bot") === 0 ||
      lower.indexOf("hello bot") === 0
    ) {
      try {
        const sticker1 = sticker[Math.floor(Math.random() * sticker.length)];
        api.sendMessage("Hi i'm always here", threadID, (err) => {
          if (err) return;
          setTimeout(() => {
            api.sendMessage({ sticker: sticker1 }, threadID);
          }, 100);
        }, messageID);
      } catch (err) {
        console.error("[ano] Error:", err.message);
      }
    }
  }
};
