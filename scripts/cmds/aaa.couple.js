...
const BG_PATH    = path.join(CANVAS_DIR, "seophi.png");

const BG_URLS = [
  "https://i.imgur.com/hmKmmam.jpg",
  "https://i.postimg.cc/hmKmmam/seophi.jpg",
];
...
async function downloadBG() {
  ...
  for (const url of BG_URLS) {
    try {
      ...
      console.log("[couple] BG downloaded:", url);
      return true;
    } catch (e) {
      console.warn("[couple] BG fail:", url, e.message);
    }
  }
  return false;
}
...
async function makeImage({ one, two }) {
  await fs.ensureDir(CANVAS_DIR);
  const pathImg   = path.join(CANVAS_DIR, `couple_${one}_${two}.png`);
...
module.exports = {
  config: {
    name: "couple",
    version: "2.0.0",
    author: "Seo Phi - Convert by Siegfried Samá",
    countDown: 5,
    role: 0,
    description: { en: "I-ship ang dalawang tao 💕" },
    category: "img",
    guide: { en: "{pn} @mention" },
  },
  ...
  onStart: async function ({ api, event, usersData }) {
    ...
    if (!mentions[0])
      return api.sendMessage("❗ I-mention ang taong gusto mong i-ship!", threadID, messageID);
    ...
    try {
      ...
      await api.sendMessage(
        {
          body: `💕 Ship! ${senderName} + @${targetName} 💕`,
